...
- **2-Phase Ingestion Hang Points:** Phase 1b (DeepSeek extraction) commonly exceeds 120s timeout on large documents (27-page test showed 153.7s); system has retry/backoff logic that eventually succeeds. Phase 2 fires annotation concurrently via `asyncio.gather()` (bounded by `_annot_semaphore`), so total Phase 2 time is ~max(question_annotation_latency) + serial_validate_persist_loop_time; typical 40-50 question extraction takes 3-8 minutes Phase 2 depending on annotation latency.
