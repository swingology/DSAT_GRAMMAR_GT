...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const loginWithGoogle = useCallback(async (credential: string) => {
    ...
    // The token response carries no profile — only /me does. Any registered user can
    // sign in here; RequireAdmin decides whether they may see the admin UI.
    const me = await authApi.me()
    storeProfile(me)
    setProfileState(me)
    setStatus('authenticated')
  ...
  const logout = useCallback(async () => {
    ...
    // Drop cached admin data so it isn't served to whoever signs in next.
    queryClient.clear()
  }, [queryClient])
...
// eslint-disable-next-line react-refresh/only-export-components -- hook and provider live together, matching the student app
