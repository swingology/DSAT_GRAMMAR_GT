...
export function QuestionIdBadge({ id, className = '' }: QuestionIdBadgeProps) {
  return (
    <div className={`flex flex-wrap items-start gap-2 ${className}`}>
      <div
        className="inline-flex max-w-full items-start gap-1 rounded border border-amber-200 bg-amber-50 px-2 py-1 text-xs font-medium text-amber-800"
      ...
      <ReportIssueButton questionId={id} />
    </div>
  )
}
