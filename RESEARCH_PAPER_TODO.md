...
## Verified Benchmark Gaps (2026-09-15)

The production pipeline cannot stand in for the controlled benchmark. Each gap below was verified against the code on 2026-09-15 and maps onto the checklist in "Required Code Work Before the Pilot"; every box there is still unchecked.

### 1. Condition isolation
- No `rule_mode`, `example_mode`, or `generation_version` flag exists anywhere in `backend/app`.
- `backend/app/prompts/generate_prompt.py` always injects rules; examples come from auto-selection in `backend/app/routers/generate.py::_select_source_question_ids_for_batch`.
- Version 2 (no rules, no examples) and Version 3 (rules only) cannot be produced today.

### 2. Model routing
- `litellm/config.yaml` maps `gpt-4o`, `claude-sonnet-4-6`, and `deepseek-v4-pro:cloud` all to the same local `ollama_chat/qwen3.6:27b`.
- A four-model comparison through the current stack would compare one model to itself three times.
- Required: direct hosted endpoints plus a routing probe that fails when two model IDs resolve to one backend.

### 3. Reproducibility metadata
- Providers (`backend/app/llm/openai_provider.py`, `backend/app/llm/ollama_provider.py`) record input and output tokens only.
- Nothing records: returned model name, cached-input tokens, reasoning tokens, prompt hash, rules hash, examples hash, or repository commit.
- The stored seed is never passed to the provider. Runs are stochastic repetitions, not reproducible.

### 4. Reviewer study
- `backend/app/review/runner.py` excludes the generator's provider, so the self-preference test needs a separate benchmark configuration that deliberately includes those pairings.
- `backend/app/prompts/review_prompt.py` has seven scores and no correctness or answer-uniqueness score. The human rubric must be built from scratch.

### 5. Containment
- Benchmark outputs must stay out of the production `questions` table and student retrieval. No such partition exists.
- `backend/benchmark/` does not exist.

### What does not need a benchmark
The existing consensus gate (`backend/app/review/consensus.py`), review scores, and admin overrides are subjects of the study, not evidence. They can be measured against the human reference once it exists.

### Shortest path
Build only the pilot-sized harness:

- [ ] Fixed manifest of target specifications with stable `specification_id`.
- [ ] Explicit `generation_version` / `rule_mode` / `example_mode` flags with regression tests proving each condition includes and excludes the intended prompt sections.
- [ ] Append-only result record per generation (see "Minimum Result Record" below).
- [ ] Routing probe that fails when two supposedly distinct model IDs resolve to the same backend.
- [ ] Benchmark output partition that cannot become student-visible.

Everything else in "Proposed Benchmark Layout" can wait until the pilot shows it is needed.
