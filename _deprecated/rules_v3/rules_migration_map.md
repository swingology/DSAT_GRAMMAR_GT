# Rules v3 Migration Map

This file maps the original working rule files into the v3 layout. It exists to
...
The v3 files reorganize them; they do not intentionally rename production keys
or change validator semantics.

## v3 File Responsibilities

| v3 file | Responsibility | Source sections |
|---|---|---|
...
| `rules_dsat_grammar_module.md` | grammar taxonomy, all syntactic trap keys (13 approved values), passage construction rules (all focus keys including 14 newly filled templates), distractor heuristics (all focus keys including 7 EOI families), evidence span rules (grammar), grammar validation, grammar generation, no-change rules, tense/register metadata, complete worked examples, **v3 additions: Boundaries Decision Tree (§25), FSS Edge Case Appendix (§26), Authenticity Anti-Patterns (§27), DSAT Trends (§28)** | grammar 3-13, 17-20, grammar-specific parts of 21-29, complete examples from 20.5; v3 additions from external research synthesis (see research_report_v3.md) |
...
Before using v3 as the only LLM rule source in production prompts, confirm:
...
- [ ] a mixed-domain ingestion prompt using all three v3 files correctly routes
...
Load all three v3 rule files, then require the model to:
...
v3 extends v2 with externally-researched enhancements. The research basis is documented in `research_report_v3.md`.

### New Sections in Grammar Module

| Section | Title | Source |
| ------- | ----- | ------ |
| §18.1 | Transition Logic — Extended Heuristics | Bluebook frequency analysis, prep expert taxonomies |
| §18.2 | Transition Word Frequency Table | Bluebook Tests 4-10 frequency analysis |
| §18.3 | Rhetorical Synthesis — Goal Types & Hard Traps | June 2025 DSAT guide, Dec 2025 "Smart Student" advisory |
| §18.4 | Redundancy/Concision — Patterns & Tables | PrepScholar, Grammarmasterkey |
| §18.5 | Precision/Word Choice | UWorld, prep expert analyses |
| §18.6 | Register/Style Consistency | College Board framework |
| §18.7 | Emphasis/Meaning Shifts | DSAT strategy guides |
| §18.8 | Data Interpretation Claims | College Board quantitative evidence specs |
| §25 | Boundaries Decision Tree & Anti-Trap Rules | Test Innovators, NUM8ERS, multiple prep sources |
| §26 | Form/Structure/Sense Edge Case Appendix | UWorld, PrepScholar, The Test Advantage |
| §27 | Authenticity Anti-Patterns | ChatSAT Benchmark Study (May 2025), InGenius Prep (Feb 2026) |
| §28 | 2025-2026 DSAT Trends | Gateplus analysis, College Board framework v3.01 |

### Expanded Passage Construction Rules (§17)

Previously missing templates now filled: `voice_active_passive`, `negation`, `noun_countability`, `determiners_articles`, `affirmative_agreement`, `elliptical_constructions`, `conjunction_usage`, `possessive_contraction`, `quotation_punctuation`, `logical_predication`, `precision_word_choice`, `register_style_consistency`, `emphasis_meaning_shifts`, `data_interpretation_claims`, and rhetorical synthesis passage construction.

### Compatibility Note

All v3 additions are backwards-compatible with v2. No approved keys were renamed or removed. The v3 grammar module is a strict superset of v2. Loading v3 where v2 was previously loaded produces identical classification output for all existing items; new heuristics and edge cases only affect generation quality and edge-case classification accuracy.
