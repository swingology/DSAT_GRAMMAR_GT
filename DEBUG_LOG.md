...
- **Medium:** `student_failure_mode_key` D.7 drift — ontology.py has 63 keys, D.7 in the grammar rules doc lists only 37. The 26 missing keys are reading/data-analysis failure modes (`evidence_scope_mismatch`, `wrong_row_column_lookup`, `individual_from_aggregate`, `wrong_group_selected`, `control_group_misidentification`, `subgroup_overgeneralization`, `polarity_blindness`, `negation_blindness`, `connotation_surface_match`, `local_role_misread`, `register_tone_blindness`, `figurative_meaning_blindness`, `exact_value_misread`, `absolute_value_overweighting`, `all_measures_not_checked`, `wrong_comparison_direction`, `single_measure_overread`, `local_maximum_overread`, `constraint_ignored`, `two_part_claim_partial_match`, `attribution_swap`, `author_action_overread`, `scope_role_confusion`, `agreement_degree_overread`, `relationship_simplification`, `rhetorical_verb_partial`, `parenthetical_function_confusion`). Added to ontology.py but never backfilled to D.7. LLM won't know these keys exist when using the rules doc for annotation. **Needs rules edit** to append a "Reading / Data-Analysis Failure Modes" subsection to D.7.
